//! Custom provider adapter — user-defined endpoint
...
pub async fn chat(req: &ChatRequest) -> Result<ChatResponse> {
    ...
    let base = req.base_url.as_deref().unwrap_or("http://localhost:8080/v1");
    ...
    parse_openai_response(&data, "custom")
}

pub async fn validate_key(api_key: &str, base_url: Option<&str>) -> Result<KeyValidationResult> {
    let client = reqwest::Client::new();
    let base = base_url.unwrap_or("http://localhost:8080/v1");
    let resp = client
    ...
    match resp {
        Ok(r) if r.status().is_success() => Ok(KeyValidationResult {
            valid: true, provider: "custom".into(), models_count: None, error: None,
        }),
        Ok(r) => Ok(KeyValidationResult {
            valid: false, provider: "custom".into(), models_count: None,
...
pub async fn fetch_models(api_key: &str, base_url: Option<&str>) -> Result<Vec<ModelInfo>> {
    let client = reqwest::Client::new();
    let base = base_url.unwrap_or("http://localhost:8080/v1");
    let response = client
        .get(&format!("{}/models", base))
    ...
    let models = data["data"].as_array()
        .map(|arr| arr.iter().filter_map(|m| {
            let id = m["id"].as_str()?;
            Some(ModelInfo {
                id: id.to_string(), name: id.to_string(), provider: "custom".into(),
